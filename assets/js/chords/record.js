...
function playNotesAux(notesIndex, speed) {
    if (speed === playSpeed && !playBtnDown) {
        ...
        if (notesIndex.length > 0 && !recordBtnDown) {
            keyClick.call(pianoKeys[notesIndex[0]], null, true);
            setTimeout(() => {
                return playNotesAux(notesIndex.slice(1), speed);
            }, speed);
...
function main() {
    ...
    // Piano Code
    recordBtn.addEventListener("click", clickPlay);
    playBtn.addEventListener("click", clickFastPlay);
