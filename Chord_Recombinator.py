...
For l in list(range(3,len(notes)+1)):
...
For c in combos:
        ...
        l_pc_set = {pc_set[i]:c[i]fori inrange(len(c))}

        #print(list(l_pc_set.keys()))

        # below, we sort pc_set by ascending order
        # full name is sorted_pc_set
        s_pc_set =sorted(pc_set)

        #print(s_pc_set)

        # this list stores the notes in order.
        chord = {l_pc_set.get(s_pc_set[i]):s_pc_set[i]fori inrange(len(pc_set))}
        ...
        for s in range(len(storage)):
                ...
                inv = {pos[i]:chord[pos[i]]fori inrange(len(pos))}# inv is inversion
                #print(inv)
                ...
                ifdiff_last <0:
                        # takes complement of interval difference
                ...
                if counter ==0:
                        ...
                        # since there is a tie, we take the first and second-last
                        # notes' interval. 
                        differences = []
                        ...
                        # new chord < old chord == new chord
                                ...
                                ifp_i_num_list[0] <inv_num_list[0]:
                                        #print("Old chord wins!")
                                        normal_form =prev_inv
                                # new chord's first note is the lowest
                                elifinv_num_list[0] <p_i_num_list[0]:
        ...
        normal_form_n = list(normal_form.values())# normal form, with numbers only
        ...
        p_t_n_form =list(t_normal_form.keys())# transposed normal form w/ pitches
        n_t_n_form =list(t_normal_form.values())# transposed normal form w/ numbers
        ...
        if not chord_identified:
                ...
                if chord_name =="Sus or Quartal Chord":
                        print(f"Chord is:{root}Sus2, {normal_form_p[2]} Sus4 or {normal_form_p[1]} quartal chord")
                        print(f"Notes:{normal_form_p}")
